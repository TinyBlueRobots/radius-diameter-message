...
func Test_diameter_message(t *testing.T) {
	...
	actual, err := diameter.ReadMessage(bytes)
	if err != nil {
		t.Fatal(err)
	}
	
	avp := actual.Avps.GetFirst(258, 0)
	assert.Equal(t, uint32(1), *avp.ToUint32())
	avp = actual.Avps.GetFirst(257, 0)
...
func Test_diameter_read_grouped_avp(t *testing.T) {
	...
	actual, err := diameter.ReadMessage(messageData)
	if err != nil {
		t.Fatal(err)
	}

	apn := actual.Avps.GetFirst(873, 10415).ToGroup().GetFirst(874, 10415).ToGroup().GetFirst(30, 0).ToString()
...
func Test_diameter_write_grouped_avp(t *testing.T) {
	...
	avp := actual.Avps.GetFirst(456, 0).ToGroup().GetFirst(432, 0)
	assert.Equal(t, uint32(1), *avp.ToUint32())
}
...
func Test_diameter_timestamp(t *testing.T) {
	...
	actualTime := actual.Avps.GetFirst(55, 0).ToTime().UTC()
	expected := time.Date(2024, time.May, 15, 16, 50, 37, 0, time.UTC)
	assert.Equal(t, expected, actualTime)
}
